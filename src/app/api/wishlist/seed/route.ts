...
export async function GET(request: Request) {
  try {
    ...
    logs.push(`Found ${stickyNotes.length} active sticky notes.`);
    ...
    if (stickyNotes.length > 2) {
      ...
      logs.push(`Deactivated ${toRemove.length} duplicate sticky notes.`);
    }

    // 2. Add/update student products directly
    let added = 0;
    let updated = 0;
