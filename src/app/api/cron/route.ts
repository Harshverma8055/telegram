...
export async function GET(request: Request) {
  ...
  try {
    ...
    // =====================================================================
    // Stage 4: Automated Wishlist Posting for Student Channel (@hosteldeals)
    // =====================================================================
    if (!isSilent) {
      try {
        const lastStudentPost = await prisma.deal.findFirst({
          where: { dealType: 'wishlist_publish' },
          orderBy: { publishedAt: 'desc' }
        });

        let timeSinceLastStudentPostMin = 999;
        if (lastStudentPost && lastStudentPost.publishedAt) {
          timeSinceLastStudentPostMin = (Date.now() - new Date(lastStudentPost.publishedAt).getTime()) / (1000 * 60);
        }

        // Automatically post a wishlist item every 60 minutes
        if (timeSinceLastStudentPostMin >= 60) {
          const nextProducts = await prisma.$queryRaw<any[]>`
            SELECT * FROM "WishlistProduct" 
            WHERE "wishlist" = true AND "price" > 0 AND "mrp" > 0
            ORDER BY "priority_score" DESC 
            LIMIT 1
          `;

          if (nextProducts.length > 0) {
            const prod = nextProducts[0];
            const studentChannel = process.env.TELEGRAM_STUDENT_CHANNEL || '@hosteldeals';
            
            console.log(`🤖 Auto-publishing Wishlist product "${prod.title.substring(0, 30)}" to ${studentChannel}`);
            ...
            if (bot) {
              ...
              const inlineKeyboard = {
                inline_keyboard: [[{ text: buttonText, url: affiliateUrl }]]
              };

              if (prod.image && prod.image.startsWith('http')) {
                await bot.sendPhoto(studentChannel, prod.image, {
                  caption: caption,
                  parse_mode: 'Markdown',
                  reply_markup: inlineKeyboard
                });
              } else {
                await bot.sendMessage(studentChannel, caption, {
                  parse_mode: 'Markdown',
                  reply_markup: inlineKeyboard
                });
              }
            ...
            const mainProduct = await prisma.product.upsert({
              where: { platformId_externalId: { platformId: amazonPlatform.id, externalId: prod.asin } },
        ...
        console.error('Error in automated student channel posting:', studentErr.message);
      }
    }
